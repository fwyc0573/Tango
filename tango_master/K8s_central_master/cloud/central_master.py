...
def init_Graph_data_x():
    ...
    start_nodes, end_nodes, master_index_dict, master_index_dict_rev, num_id_now = [], [], {}, {}, 0
...
def get_observation(req, tmp_resources_on_cluster, tmp_tasks_execute_situation_on_each_node,
    ...
    for i in range(len(master_index_dict_rev)):
        ...
        if node_name in EDGEMASTER_IP_DICTSET:  # i.e., master node
            ...
            LC_CPU_SH = tmp_resources_on_cluster[map_master][node_name]["cpu"]["LC_CPU_SH"]
            BE_CPU_SH = tmp_resources_on_cluster[map_master][node_name]["cpu"]["BE_CPU_SH"]
            TOTAL_INIT = tmp_resources_on_cluster[map_master][node_name]["cpu"]["TOTAL_INIT"]
            ...
            FREE_MEO = tmp_resources_on_cluster[map_master][node_name]["meo"]["FREE_MEO"]
            REAL_LC_CPU = 0
            REAL_BE_CPU = 0

            for service_id in tmp_resources_on_each_node_dict[map_master][node_name]:
                if str(service_id) in LC_SERVICE_LIST:
                    REAL_LC_CPU += tmp_resources_on_each_node_dict[map_master][node_name][service_id]["cpu_use"]
                else:
                    REAL_BE_CPU += tmp_resources_on_each_node_dict[map_master][node_name][service_id]["cpu_use"]

            res_cpu_need_sum = 0
            for service_id in tmp_tasks_execute_situation_on_each_node[map_master][node_name]:
                if service_id not in LC_SERVICE_LIST:
                    res_cpu_need_sum += tmp_tasks_execute_situation_on_each_node[map_master][node_name][service_id][
                                            'ST'] * BE_CPU_ONCE_NEED_DICT[service_id]

            just_need = [LC_CPU_SH / 100, BE_CPU_SH / 100, FREE_BE_CPU_SH / 100, LC_MEO, BE_MEO, FREE_MEO,
                         REAL_LC_CPU / 100, REAL_BE_CPU / 100, res_cpu_need_sum / 100]
...
def THR_meomery_push(manager_resources_on_cluster, manager_task_dict, manager_resources_on_each_node_dict, memory,
                     reward_back_queue, total_numsteps, dict_lock, meomery_dict, memory_lock):
    record_delet_id = 0
    reward_record = []
    while True:
        if reward_back_queue.qsize() > 10:
            for i in range(10):
                ...
                try:
                    req, action, state, mask = meomery_dict[task_id]["req"], meomery_dict[task_id]["ac"], \
                                               meomery_dict[task_id]["st"], meomery_dict[task_id]["ma"]

                    next_state = get_observation(req, manager_resources_on_cluster, manager_task_dict,
...
def centralized_scheduling_dispatcher(cache_queue, edge_queue_dict, manager_resources_on_cluster, manager_task_dict,\
                                manager_resources_on_each_node_dict, reward_back_queue, tmp_lock):
    """
    ...
    my_action_space_sample = spaces.Box(low=0.01, high=1, shape=(len(master_index_dict),1), dtype=np.float32)
    ...
    while True:
        ...
        while still_do:
            ...
            tmp_resources_on_cluster = valueclone_nested_dict_proxy(manager_resources_on_cluster)
            tmp_tasks_execute_situation_on_each_node = valueclone_nested_dict_proxy(manager_task_dict)
            tmp_resources_on_each_node_dict = valueclone_nested_dict_proxy(manager_resources_on_each_node_dict)

            state = get_observation(div_req, manager_resources_on_cluster, tmp_tasks_execute_situation_on_each_node, 
                                    manager_resources_on_each_node_dict)
            ...
            div_req[-1], div_req[4], div_req[2] = node_target, GNN_kind + "-SAC", str(task_id)
            ...
            tmp_tasks_execute_situation_on_each_node = valueclone_nested_dict_proxy(manager_task_dict)
            next_state = get_observation(div_req, manager_resources_on_cluster, tmp_tasks_execute_situation_on_each_node
                                         , manager_resources_on_each_node_dict)
            reward = sac_calcureward(tmp_tasks_execute_situation_on_each_node, node_target,
                                       manager_resources_on_each_node_dict)
...
def sac_calcureward(tmp_tasks_execute_situation_on_each_node, my_choice_node, tmp_resources_on_each_node_dict, check=False):
    node_resNeed_list = []
    node_resNeed_dict = {}
    same_as_me_dict = {}
    ...
    for master in tmp_tasks_execute_situation_on_each_node:
        for node in tmp_tasks_execute_situation_on_each_node[master]:
            ...
            for service_id in tmp_tasks_execute_situation_on_each_node[master][node]:
                if service_id not in LC_SERVICE_LIST:
                    res_cpu_need_sum += tmp_tasks_execute_situation_on_each_node[master][node][service_id]['ST'] * \
        ...
        if node_resNeed_dict[node] == my_choice_resNeed:
            ...
            for service_id in tmp_resources_on_each_node_dict[map_master][node]:
                if str(service_id) not in LC_SERVICE_LIST:
                    REAL_BE_CPU += tmp_resources_on_each_node_dict[map_master][node][service_id]["cpu_use"]
    ...
    if my_choice_resNeed == 0:
        ...
        if same_as_me_dict[my_choice_node] == 0:
            ...
            order_list_tuple = sorted(same_as_me_dict.items(), key=lambda s: s[1], reverse=True)
            order_node_name = [j for i in order_list_tuple for j in i if type(j) == str]
            rank_num = len(node_resNeed_dict) - len(order_node_name) + order_node_name.index(my_choice_node) + 1
...
def classificate_request_from_edge(cache_queue, manager_resources_on_cluster,collect_tasks_execute_situation_on_each_node, manager_resources_on_each_node_dict, edge_queue_dict):
    while True:
        req = cache_queue.get()
        pickle.dumps(req)


def async_BE_threadExecute(req, manager_resources_on_cluster, collect_tasks_execute_situation_on_each_node, manager_resources_on_each_node_dict, myCPU_lock_dict, tmp_write_lock):
...
def sendtoMyself():
    ...
    detail_mission = {'name': "test1", 'service_type': "test1", 'success': 55, 'failure': 55}
...
def collect_data_excute(collect_data_queue, mdata_lock, managerState, manager):
    while True:
        ...
        thr = threading.Thread(target=THR_collect_data, args=(mission, manager, mdata_lock, managerState))
        thr.start()
...
def THR_collect_tasks_execute_situation_on_each_node(pool_args):
    ...
    mission = json.loads(mission.decode('utf-8')) # masterName,service_type,successNum,failureNum
    ...
    masterName, nodeName, service_id, success, stuck, failure = mission['masterName'], mission['nodeName'], str(mission['service_id']), int(mission['success']), int(mission['stuck']), int(mission['failure'])

    if success != 0:
        if stuck != 0:
            with task_lock_dict[nodeName][service_id]['SU'], task_lock_dict[nodeName][service_id]["ST"]:
        ...
        if stuck != 0:
            with task_lock_dict[nodeName][service_id]['F'], task_lock_dict[nodeName][service_id]["ST"]:
...
def collect_tasks_execute_situation_on_each_node_excute(collect_tasks_execute_situation_queue, task_lock_dict, manager_task_dict, manager):
    pool = ThreadPoolExecutor()
    while True:
        mission = collect_tasks_execute_situation_queue.get()
        task = pool.submit(THR_collect_tasks_execute_situation_on_each_node, (mission, task_lock_dict, manager_task_dict))
...
def THR_resources_on_each_node_excute(pool_args):
    ...
    print("listening:", valueclone_nested_dict_proxy(manager_resources_on_each_node_dict))


def resources_on_each_node_excute(resources_on_each_node_queue, manager_resources_on_each_node_dict, share_lock4):
    pool = ThreadPoolExecutor()
    ...
    while True:
        ...
        task = pool.submit(THR_resources_on_each_node_excute, (resources, share_lock4, manager_resources_on_each_node_dict))
        task.add_done_callback(thread_pool_callback)
...
def collect_cluster_info_excute(collect_cluster_info_queue, manager_resources_on_cluster, share_lock5):
    pool = ThreadPoolExecutor()

    while True:
        resources = collect_cluster_info_queue.get()
        task = pool.submit(THR_collect_cluster_info_excute, (resources, share_lock5, manager_resources_on_cluster))
        task.add_done_callback(thread_pool_callback)
...
def count_task_num(task_dict):
    ...
    return count_num_lc_su, count_num_lc_f, count_num_be_su, count_num_be_f, count_num_lc_st, count_num_be_st


def csv_collect_everthing(manager_resources_on_each_node_dict, manager_resources_on_cluster, manager_task_dict, req_load_dict):
    ...
    while True:
        ...
        docker_stats_dict = valueclone_nested_dict_proxy(manager_resources_on_each_node_dict)
        manager_resources_on_cluster_copy = valueclone_nested_dict_proxy(manager_resources_on_cluster)
        ...
        for master in docker_stats_dict:
            for node in docker_stats_dict[master]:
                for service_id in docker_stats_dict[master][node]:
                    if str(service_id) in LC_SERVICE_LIST:
                        sum_lc_cpu_use += docker_stats_dict[master][node][service_id]["cpu_use"]
                        sum_lc_meo_use += docker_stats_dict[master][node][service_id]["meo_use"]
                    else:
                        sum_be_cpu_use += docker_stats_dict[master][node][service_id]["cpu_use"]
                        sum_be_meo_use += docker_stats_dict[master][node][service_id]["meo_use"]

        for master in manager_resources_on_cluster_copy:
            for node in manager_resources_on_cluster_copy[master]:
                sum_lc_cpu_sh += manager_resources_on_cluster_copy[master][node]["cpu"]["LC_CPU_SH"]
                sum_be_cpu_sh += manager_resources_on_cluster_copy[master][node]["cpu"]["BE_CPU_SH"]
                sum_lc_meo_sh += manager_resources_on_cluster_copy[master][node]["meo"]["LC_MEO"]
                sum_be_meo_sh += manager_resources_on_cluster_copy[master][node]["meo"]["BE_MEO"]

        
        if counters % 50 == 0 and counters != 0:
            record_lc_success, record_lc_failure, record_be_success, record_be_failure, _, _ = count_task_num(task_dict)
            counters += 1
            continue
            
        lc_success_all, lc_failure_all, be_success_all, be_failure_all, _, _ = count_task_num(task_dict)
        ...
        if record_load["LC"] >= 0:
            ...
            lc_cpu_use_rate = round(sum_lc_cpu_use / (total_node_num * CPU_TOTAL_INIT), 5)
            be_cpu_use_rate = round(sum_be_cpu_use / (total_node_num * CPU_TOTAL_INIT), 5)
            ...
            lc_meo_use_rate = round(sum_lc_meo_use / (total_node_num * MEO_TOTAL_INIT), 5)
            be_meo_use_rate = round(sum_be_meo_use / (total_node_num * MEO_TOTAL_INIT), 5)
            
            req_center_res_rate = round(sum(list(record_load.values())) / 100, 2)
            record_list = [lc_cpu_use_rate, be_cpu_use_rate, lc_cpu_sh_rate, be_cpu_sh_rate, lc_meo_use_rate, \
                            be_meo_use_rate, lc_success_rate, be_success_rate, req_center_res_rate, lc_meo_sh_rate, \
                            be_meo_sh_rate, time_now]
